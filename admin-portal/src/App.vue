...
<template>
  <div class="admin-container">
    ...
    <div class="monitor-grid">
      ...
      <!-- Card 2: Database Cluster Management Workshop -->
      <div class="card">
        ...
        <div class="form-group">
          ...
          <div class="input-group">
            <input id="db-name-input" v-model="targetDbName" type="text"
              placeholder="e.g., ogame_game_db, ogame_dev_sandbox" :disabled="loading" />
        ...
        <!-- Action 2: Trigger SQL Blueprint Migrations & Admin Provisioning -->
        <div class="migration-block">
          <h4>Schema Blueprint & Admin Provisioning</h4>
          <p class="footnote">Populate the current active database context and register the master administrator
            account.</p>

          <div class="form-group sub-form">
            <div class="input-field-sub">
              <label for="admin-user-input">Master Admin Username:</label>
              <input id="admin-user-input" v-model="adminUsername" type="text" placeholder="e.g., admin_commander"
                :disabled="loading || isDbDisconnected" />
            </div>
            <div class="input-field-sub" style="margin-top: 10px;">
              <label for="admin-pass-input">Master Admin Password:</label>
              <input id="admin-pass-input" v-model="adminPassword" type="password"
                placeholder="Minimum 6 characters securely encrypted" :disabled="loading || isDbDisconnected" />
            </div>
          </div>

          <button @click="executeSchemaMigration"
            :disabled="loading || isDbDisconnected || !adminUsername || !adminPassword" class="btn-accent"
        ...
        <table v-else class="catalog-table">
          ...
          <tbody>
            <tr v-for="db in dbCatalog" :key="db.name" :class="{ 'active-row': db.is_active }">
              ...
              <td>
                <button @click="quickSwitch(db.name)" :disabled="loading || db.is_active" class="btn-small">
                  Hot-Swap Route
                </button>
              </td>
            </tr>
...
export default defineComponent({
  ...
  setup() {
    const systemStatuses = ref({
      ...
      postgres: { status: false, version: 'PostgreSQL 16', uptime: 'N/A', database_state: '' },
    ...
    const isDbDisconnected = computed(() => {
      return !systemStatuses.value.postgres.database_state;
    });
    ...
    const checkClusterHealth = async () => {
      ...
      try {
        ...
        systemStatuses.value.postgres = { status: false, version: 'PostgreSQL 16', uptime: 'N/A', database_state: '' };
    ...
    const executeSchemaMigration = async () => {
      ...
      } catch (e) { operationSuccess.value = false; operationMessage.value = 'Failed to transmit migration pipeline packets.'; } finally { loading.value = false; }
    }; onMounted(() => {
      checkClusterHealth();
    ...
    return { systemStatuses, dbCatalog, targetDbName, adminUsername, adminPassword, loading, operationMessage, operationSuccess, isDbDisconnected, formatServiceName, executeDbSwitch, quickSwitch, executeSchemaMigration };
  }
});
</script>
