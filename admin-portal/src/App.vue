...
<template>
  <div class="admin-container">
    ...
    <!-- Row 1: Infrastructure Telemetry Health Board & Database Workshop -->
    <div class="monitor-grid">
      ...
      <div class="card">
        ...
        <div class="services-list">
          <div class="service-item-extended" v-for="(metrics, name) in systemStatuses" :key="name">
            ...
            <!-- Dynamic sub-metrics block -->
            <div class="service-metrics-row" v-if="metrics.status">
              <!-- Render version details for standard services -->
              <span class="metric-tag" v-if="name !== 'postgres' && name !== 'redis'">
                Version: <strong>{{ metrics.version }}</strong>
              </span>
              
              <!-- Render dynamic database routing metadata with color alerts -->
              <span class="metric-tag" v-if="name === 'postgres'">
                Active Target DB: 
                <strong :style="{ color: metrics.version === 'DISCONNECTED' ? '#f59e0b' : '#34d399' }">
                  {{ metrics.version }}
                </strong>
              </span>
              
              <!-- Render uptime counters -->
              <span class="metric-tag" v-if="name !== 'postgres' && name !== 'redis'">
                Uptime: <strong>{{ metrics.uptime }}</strong>
              </span>
              <span class="metric-tag" v-if="name === 'redis'">
                Bridge: <strong>{{ metrics.uptime }}</strong>
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="card">
        ...
        <!-- Action 1: Create or Switch Input Block -->
        ...
        <!-- Action 2: Trigger SQL Blueprint Migrations -->
  <div class="migration-block">
    ...
    Execute SQL Schema Migration
        ...
        <!-- Operation Status Banner Notifications -->
  ...
  <script lang="ts">
    ...
    fetchDatabaseCatalog();
      ...
      checkClusterHealth();
fetchDatabaseCatalog();}, 2000);});onBeforeUnmount(() => {if (pollingInterval) clearInterval(pollingInterval);});return {systemStatuses, dbCatalog, targetDbName, loading, operationMessage, operationSuccess, isDbDisconnected, formatServiceName, executeDbSwitch, quickSwitch, executeSchemaMigration};}});.admin-container {padding: 30px;font-family: 'Segoe UI', Roboto, sans-serif;background: #0f172a;color: #f8fafc;min-height: 100vh;}h1 {color: #38bdf8;border-bottom: 2px solid #1e293b;padding-bottom: 15px;margin-bottom: 30px;}.monitor-grid {display: grid;grid-template-columns: repeat(auto-fit, minmax(380px, 1fr));gap: 25px;}.card {background: #1e293b;padding: 24px;border-radius: 8px;box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.4);border: 1px solid #334155;}h3 {margin - top: 0;color: #f1f5f9;border-bottom: 1px solid #334155;padding-bottom: 10px;}.section-desc {font - size: 13px;color: #94a3b8;margin-top: -5px;margin-bottom: 20px;}.services-list {display: flex;flex-direction: column;gap: 14px;}.service-item-extended {background: #0f172a;border-radius: 6px;padding: 12px;border-left: 4px solid #38bdf8;}.service-main-row {display: flex;justify-content: space-between;align-items: center;}.service-name {font - weight: 600;font-size: 14px;letter-spacing: 0.05em;}.service-metrics-row {display: flex;gap: 20px;margin-top: 8px;padding-top: 8px;border-top: 1px solid #1e293b;font-size: 12px;color: #94a3b8;}.status-badge {padding: 4px 10px;border-radius: 4px;font-size: 11px;font-weight: bold;}.status-badge.online {background: #064e3b;color: #34d399;border: 1px solid #059669;}.status-badge.offline {background: #7f1d1d;color: #fca5a5;border: 1px solid #dc2626;}/* Forms and Buttons Styles */.form-group {margin - bottom: 20px;}label {display: block;font-size: 13px;color: #94a3b8;margin-bottom: 6px;}.input-group {display: flex;gap: 10px;}input {flex: 1;background: #0f172a;border: 1px solid #334155;padding: 10px;border-radius: 4px;color: #fff;font-size: 14px;}input:focus {border - color: #38bdf8;outline: none;}.btn-primary {background: #0284c7;color: #fff;border: none;padding: 10px 16px;border-radius: 4px;cursor: pointer;font-weight: 600;}.btn-primary:hover:not(:disabled) {background: #0369a1; }.btn-accent {background: #d97706;color: #fff;border: none;padding: 12px 20px;border-radius: 4px;cursor: pointer;font-weight: 600;width: 100%;}.btn-accent:hover:not(:disabled) {background: #b45309; }button:disabled {background: #475569 !important;color: #94a3b8 !important;cursor: not-allowed;}.migration-block {margin - top: 25px;padding-top: 20px;border-top: 1px solid #334155;}h4 {margin: 0 0 5px 0; color: #cbd5e1; }.footnote {font - size: 12px; color: #64748b; margin-bottom: 12px; }.status-banner {margin - top: 15px;padding: 10px;border-radius: 4px;font-size: 13px;}.success-banner {background: #064e3b; color: #34d399; border: 1px solid #059669; }.error-banner {background: #7f1d1d; color: #fca5a5; border: 1px solid #dc2626; }/* Catalog Inventory Table Styles */.catalog-table {width: 100%;border-collapse: collapse;margin-top: 15px;font-size: 14px;}th, td {text - align: left;padding: 12px;border-bottom: 1px solid #334155;}th {color: #94a3b8; font-weight: 600; }.active-row {background: rgba(56, 189, 248, 0.08); }.db-name-cell {font - family: monospace; font-size: 15px; }.badge-indicator {padding: 2px 8px;border-radius: 4px;font-size: 11px;font-weight: bold;}.badge-active {background: #064e3b; color: #34d399; }.badge-idle {background: #334155; color: #94a3b8; }.btn-small {background: #334155;color: #e2e8f0;border: 1px solid #475569;padding: 6px 12px;border-radius: 4px;cursor: pointer;font-size: 12px;}.btn-small:hover:not(:disabled) {background: #475569; }.empty-catalog {text - align: center;padding: 30px;color: #64748b;font-style: italic;}